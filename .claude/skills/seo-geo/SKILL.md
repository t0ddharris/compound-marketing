...
- **[HIGH]** Before recommending SEO/GEO tooling, verify current SKUs on vendor pricing pages. Vendors launched new bundled products (Semrush One, Ahrefs Brand Radar) that don't match older Pro/Guru/Business tier structures. Use agent-browser for JS-rendered pricing pages — WebFetch often returns only the JS shell. *(Session 78, 2026-04-14)*
- **[MEDIUM]** When recommending a paid tool tier, default to the lowest tier that meets current-stage capacity (keywords tracked, projects, AI prompts). Flag one critical unverified spec for sales confirmation rather than over-buying headroom. Upgrade paths exist; unused capacity doesn't. *(Session 78, 2026-04-14)*
- **[MEDIUM]** When the target has no live web property (pre-launch, demo/fictional company, or no Search Console/GA4 access), route to the file-based GEO content audit (Step 5f) instead of crawl-, SERP-, or Search-Console-dependent steps; the file-based audit is fully self-contained. *(Session 3, 2026-05-29)*
