...
**Pair with `web-design` when the variant involves UI changes.** Test variants that ship as real HTML must still follow the brand system, interaction patterns, accessibility minimums, and anti-patterns — they're not "disposable" because they're temporary. Load `web-design` when your variant changes markup, layout, or interaction; skip it for pure copy-only tests.
...
**Output location:** `marketing/tests/[test-slug]/` — confirm the project slug with the user before creating files.
...
- **tracking-setup**: For setting up test measurement
